...
folder = '/home/samuel_lozano/Curriculum_Learning/Super_Easy_Multidigit_Addition_Decimal/'
folder_specific = '/home/samuel_lozano/Curriculum_Learning/Super_Easy_Multidigit_Addition_Decimal/v1-Equal_Frequencies/'
...
def generate_train_dataset(train_couples, size_epoch):
    ...
    selected_indices = np.random.choice(len(train_couples), size=size_epoch, replace=True)
...
def model(params, x):
    ...
    salida_1 = sum(
        params[f'v_0_1_{i}_{j}'] * carry_unit_val[f'{i}_{j}'] +
        params[f'v_1_1_{i}_{j}'] * unit_val[f'{i}_{j}']
        for i, j in [(0, 2), (1, 3)]
    )
    
    salida_2 = sum(
        params[f'v_0_2_{i}_{j}'] * carry_unit_val[f'{i}_{j}'] +
