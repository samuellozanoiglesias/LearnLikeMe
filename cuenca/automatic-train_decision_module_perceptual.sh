#!/bin/bash
# Use: nohup bash automatic-train_decision_module_perceptual.sh > logs_decision_perceptual.out 2>&1 &
#
# CLI is identical to automatic-train_decision_module.sh's target script --
# train_decision_module_perceptual.py takes no extra arguments for the
# recognizer/extractors; it pulls the frozen CNN straight out of the
# unit_extractor_perceptual checkpoint for the matching omega/study_name (see
# that script's header). Make sure you've already run
# automatic-train_extractor_modules_perceptual.sh for the omega(s) below.

cluster=cuenca
number_size=2  # Number of digits in the numbers to be added (2 for two-digit addition)
study_name=NEW_STUDY  # Must match the study_name the perceptual extractors were trained under
param_type=RI  # "WI" for wise initialization or "RI" for random initialization
model_type=vector  # "argmax" or "vector" version of the decision module
...
training_mode=all # "decision_only" (freeze extractors, original behavior) or "all" (unfreeze + jointly train extractors too)
...
PYTHON_SCRIPT="../train_decision_module_perceptual.py"
...
# NOTE: each omega swept here must have a matching
# omega_{omega:.2f} perceptual extractor run already trained (via
# automatic-train_extractor_modules_perceptual.sh) for BOTH carry_extractor
# and unit_extractor under this study_name -- otherwise
# load_extractor_module_perceptual() will raise FileNotFoundError.
...
for e_idx in "${!epsilons[@]}"; do
    ...
    for o_idx in "${!omegas[@]}"; do
        ...
        nohup python3 "$PYTHON_SCRIPT" "$cluster" "$number_size" "$study_name" "$param_type" "$model_type" "$epsilon" "$omega" "$epochs" "$fixed_variability" "$training_distribution_type" "$alpha_curriculum" "$training_mode" > "logs/decision_module_perceptual_${number_size}_${study_name}_${param_type}_${model_type}_omega-${omega_fmt}_epsilon-${epsilon_fmt}_epochs-${epochs}_fixed-${fixed_variability}_dist-${training_distribution_type}_alpha-${alpha_curriculum}_training-${training_mode}.out" 2>&1 &
