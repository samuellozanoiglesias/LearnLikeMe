...
epsilon=0.5  # Epsilon value for the training
...
for i in "${!omegas[@]}"; do
    ...
    nohup python3 "$PYTHON_SCRIPT" "$cluster" "$module_name" "$study_name" "$epsilon" "$omega" "$fixed_variability" "$early_stop" "$training_distribution_type" "$alpha_curriculum" > "logs/${module_name}_${study_name}_${omega_fmt}_${fixed_variability}_${early_stop}_${training_distribution_type}_${alpha_curriculum}.out" 2>&1 &
    sleep 5
